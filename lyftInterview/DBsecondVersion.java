...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(commandPtr < commandLines.size()){
            ...
            if(commandLine.get(0).equals("ROLLBACK")){//suppose commit and rollback are not mutually exclusive
                int commitCount = 0;//for nestedBlock
                if(commandLineQueue.isEmpty() || !commandLineQueue.peekLast().get(0).equals("COMMIT")){
                    throw new Exception("it is an illegal rollback!");
                }
                boolean seenBlock = false;
                while(commitCount > 0 && !seenBlock){
                    if(commandLineQueue.isEmpty() || commandOptionQueue.isEmpty()){
                        throw new Exception("the process queue is empty!");
                    }
                    if(commandLineQueue.peekLast().equals("BLOCK"))commitCount--;
                    if(commandLineQueue.peekLast().equals("COMMIT"))commitCount++;
            ...
            commandPtr++;
            System.out.println( commandLineQueue);
            System.out.println(commandOptionQueue);
        }
